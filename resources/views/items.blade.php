...
<div class="container_">
  <div class="cell cell-2">
    ...
    @endforeach
    @if(Auth::user()->is_admin) 
    <a class="nav-link" href="{{route('new.item')}}" ><button class="btn btn-success btn-block">Új item hozzáadása-></button></a> 
    <form action="{{ route('store.new.category') }}" method="post" enctype="multipart/form-data">
            @csrf
            <input placeholder="Új kategória..." type="text" class="form-control @error('name') is-invalid @enderror" id="name" name="name" value="{{ old('title') }}">
                @error('name')
